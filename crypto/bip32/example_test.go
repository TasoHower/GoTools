...
func ExampleNewMaster() {
	...
	key, err := NewMaster(ksrv.NewHDSeedRequest_Secp256k1, seed, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_defaultWalletLayout() {
	...
	// This gives the path:
	//   m
	masterKey, err := NewKeyFromString(ksrv.NewHDSeedRequest_Secp256k1, master)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	acct0ExtAddr, err := acct0Ext10.(*ExtendedKeyEcc).Address(&chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
	acct0IntAddr, err := acct0Int0.(*ExtendedKeyEcc).Address(&chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
