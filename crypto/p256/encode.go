...
const (
	...
	//FLAGLEN FLAG常量
	FLAGLEN = 1
	//XORYVALUELEN X或者Y坐标的长度
	XORYVALUELEN = 32
	//COMPRESSEDLEN 压缩后公钥长度
	COMPRESSEDLEN = 33
	//NOCOMPRESSEDLEN 非压缩公钥长度
	NOCOMPRESSEDLEN = 65
	//COMPEVENFLAG 压缩公钥前缀偶数
	COMPEVENFLAG = 0x02
	//COMPODDFLAG 压缩公钥前缀奇数
	COMPODDFLAG = 0x03
	//NOCOMPRESSEDFLAG 非压缩前缀
...
func deCompress(yTilde int, xValue []byte, curve *elliptic.CurveParams) (*PublicKey, error) {
	...
	return &PublicKey{X: xCoord, Y: yCoord}, nil
}

// DecodePoint 解码公钥
func DecodePoint(encodeData []byte, curve *elliptic.CurveParams) (*PublicKey, error) {
	if len(encodeData) == 0 {
	...
	case 0x02, 0x03: //compressed
		...
		return deCompress(yTilde, encodeData[FLAGLEN:FLAGLEN+XORYVALUELEN], curve)
	...
	case 0x04, 0x06, 0x07: //uncompressed
		...
		return &PublicKey{X: pubKeyX, Y: pubKeyY}, nil
